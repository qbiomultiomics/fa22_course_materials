...
dds = DESeqDataSetFromMatrix(countData = FILL IN HERE,
                             ...
                             design = ~FILL IN HERE +FILL IN HERE +FILL IN HERE)
...
?results
results = results(dds_obj, format = "DataFrame", contrast = c("age_category", "FACTOR 1", "FACTOR 2")) # this is case sensitive so be careful!
```
...
Use the logic above as well as the sig_results mask to sort the results df by log2foldchange descending (take a look at the decreasing argument). Note that this can also be done in one line by bypassing the use of order_indexes as an intermediate variable.
...
Repeat the same steps to create a sorted data frame called down_reg_results (this time select for all genes where log2foldchange is < -1).
